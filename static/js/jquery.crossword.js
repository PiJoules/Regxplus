...
*/
(function($){
	$.fn.crossword = function(entryData, verbose) {
			/*
			...
			*/
				...
				buildEntries: function() {
					...
					for (var x=1, p = entryCount; x <= p; ++x) {
						...
						for (var i=0; i < entries[x-1].length; ++i) {
							light = $(puzzCells.selector +'[data-coords="' + entries[x-1][i] + '"]');
					...
					for (var i = 1; i < entryCount; i++){
						if ($('td[data-coords="1,' + i + '"] span').length <= 0){
							$('td[data-coords="1,' + i + '"]').append('<span>' + i + '</span>');
						}
						if ($('td[data-coords="' + i + ',1"] span').length <= 0){
							$('td[data-coords="' + i + ',1"]').append('<span>' + i + '</span>');
						}
					}
			...
			var nav = {
				...
				updateByEntry: function(e, next) {
					...
					if(e.keyCode === 9 || next){
						...
						clue = $(clueLiEls.selector + '[data-position=' + activePosition + ']');
			...
			var util = {
				...
				highlightClue: function() {
					var clue;				
					$('.clues-active').removeClass('clues-active');
					$(clueLiEls.selector + '[data-position=' + activePosition + ']').addClass('clues-active');
				...
				getActivePositionFromClassGroup: function(el){
						...
						if(classes.length > 1){
							...
							e1Ori = $(clueLiEls.selector + '[data-position=' + classes[0].split('-')[1] + ']').parent().prop('id');
							e2Ori = $(clueLiEls.selector + '[data-position=' + classes[1].split('-')[1] + ']').parent().prop('id');
